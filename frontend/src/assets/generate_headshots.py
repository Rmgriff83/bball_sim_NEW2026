...
# PALETTES
# ----------------------------------------------------------------------------
# Source of truth lives in palettes.json (synced from the bball-sim-assets S3
# bucket via `npm run sync:headshots` at build time). The JS composer reads
# the same file. Editing colors / adding new entries / managing ethnicity
# profiles all happens through the admin Palette Editor; nothing is hand-
# maintained here anymore.
_PALETTE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'palettes.json')
with open(_PALETTE_FILE) as _fp:
    _PALETTES = json.load(_fp)

SKIN_TONES = _PALETTES['skin']
HAIR_COLORS = _PALETTES['hair']
EYE_COLORS = _PALETTES['eye']
LIP_COLORS = _PALETTES['lip']
ETHNICITY_PROFILES = _PALETTES['ethnicity_profiles']
