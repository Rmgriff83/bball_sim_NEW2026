 ...
 * Returns null if not present. Used by the admin variant editor to seed
 * its in-memory pieces array, by HeadshotEditorView's coach load path,
 * and by LayerContextMenu/HeadshotAdminEditorView to back the per-piece
 * color picker.
 *
 * Normalizes the variant key at the filename boundary — config fields use
 * underscores (`wide_grin`) but variant SVGs live on disk with hyphens
 * (`wide-grin.svg`). Without this, callers like LayerContextMenu's
 * `layerPieces` (which gets its variantKey from getCurrentVariantKey →
 * raw config form) miss the LAYER_CONTENT entry the moment a new
 * admin-authored variant has a hyphenated name, and the piece-color
 * swatches silently disappear. Filenames never contain underscores by
 * convention, so the replace is idempotent for already-hyphenated keys.
 */
export function getVariantSource(layerId, variantKey, audience = 'player') {
  const fileKey = variantKey == null ? variantKey : String(variantKey).replace(/_/g, '-')
  return _contentMapFor(audience)[`${layerId}/${fileKey}`] ?? null
}
...
function _renderLayer(layerId, variantKey, tokens, metaAttrs = {}, overrideContent = null, pieceOverrides = null, audience = 'player') {
  ...
  // getVariantSource handles the underscore→hyphen filename normalization
  // for string-keyed variants whose admin-authored names contain hyphens
...
export function composeSvg(config, overrides = null, audience = 'player') {
  ...
  // Filename-vs-config-key normalization (underscores in config →
  // hyphens on disk) happens inside getVariantSource, which _renderLayer
  // now goes through — so every string-keyed variant lookup converges
