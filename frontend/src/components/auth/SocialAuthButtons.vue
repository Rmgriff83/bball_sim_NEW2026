...
async function handleCredential(payload) {
  ...
  try {
    ...
    emit('error', err.response?.data?.message || t('Social sign-in failed. Please try again.'))
  } finally {
    busy.value = false
  }
}
...
async function handleApple() {
  ...
  try {
    ...
    emit('error', err?.message || t('Apple sign-in failed. Please try again.'))
  }
}
...
async function handleGoogleNative() {
  ...
  try {
    ...
    emit('error', err?.message || t('Google sign-in failed. Please try again.'))
  }
}
...
<template>
  <div class="social-auth">
    ...
    >
      ...
      <span>{{ mode === 'register' ? $t('Sign up with Apple') : $t('Sign in with Apple') }}</span>
    </button>
    ...
    >
      ...
      <span>{{ mode === 'register' ? $t('Sign up with Google') : $t('Sign in with Google') }}</span>
    </button>
