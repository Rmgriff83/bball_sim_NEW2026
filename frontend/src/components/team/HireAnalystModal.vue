...
import { PERSONNEL_POOL_KEY, ANALYST_TIERS } from '@/engine/data/personnelTiers'
...
const props = defineProps({
  ...
  analyticsFacilityLevel: { type: Number, default: 1 },
})
...
function generateCandidates() {
  ...
  // 2x 3-star
  for (let i = 0; i < 2; i++) {
    const tier = ANALYST_TIERS[3]
    results.push({ name: randomName(), tier: 3, cost: tier.cost, label: tier.label, rating: tier.rating, perks: tier.perks })
  }
  // 1x 4-star
  const tier4 = ANALYST_TIERS[4]
  results.push({ name: randomName(), tier: 4, cost: tier4.cost, label: tier4.label, rating: tier4.rating, perks: tier4.perks })
...
watch(() => props.show, async (val) => {
  if (val) {
    hiring.value = false
    // Prefer the persistent pool seeded at campaign creation (candidates carry
    // an id + headshot). Fall back to the on-demand generator for legacy saves.
    try {
      const campaign = await CampaignRepository.get(props.campaignId)
      const pool = campaign?.settings?.[PERSONNEL_POOL_KEY.analyst]
...
async function hireAnalyst(candidate) {
  ...
  try {
    ...
    const poolKey = PERSONNEL_POOL_KEY.analyst
    ...
    if (campaignStore.currentCampaign) {
      campaignStore.currentCampaign.settings = {
        ...campaignStore.currentCampaign.settings,
        analyst: campaign.settings.analyst,
      }
    }

    syncStore.markDirty()
    audio.purchase()
    toastStore.showSuccess(t('Analyst hired successfully!'))
    emit('hired')
    emit('close')
  } catch (err) {
    console.error('Failed to hire analyst:', err)
    toastStore.showError(t('Failed to hire analyst'))
  } finally {
    hiring.value = false
  }
}
      ...
      <div v-if="show" class="modal-overlay" @click.self="close">
        <div class="modal-container">
          <header class="modal-header">
            <h2 class="modal-title">{{ $t('Hire an Analyst') }}</h2>
              ...
              >
                <div class="candidate-header">
                  <div class="candidate-avatar-wrap">
                    <PersonnelAvatar :personnel="candidate" kind="analyst" :size="48" />
                  </div>
                  ...
                  >
                    ...
                    <div class="perk-text">
                      ...
                      <span v-if="!isPerkActive(candidate, perk)" class="perk-req">
                        {{ $t('Requires Analytics Facility Lv {n}', { n: requiredLevelFor(candidate, perk) }) }}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <button
                  ...
                  @click="hireAnalyst(candidate)"
                >
                  {{ tokens < candidate.cost ? $t('Insufficient Tokens') : $t('Hire Analyst') }}
                </button>
              </div>
            </div>
          </main>
